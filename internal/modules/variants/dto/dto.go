...
type ConfirmImageRequest struct {
	...
	MediaType    domain.VariantMediaType  `json:"media_type"`   // IMAGE or VIDEO
}
...
type ImageResponse struct {
	...
	URL       string                  `json:"url"`
	S3Key     string                  `json:"s3_key"`
	MediaType domain.VariantMediaType `json:"media_type"`
	SortOrder int                     `json:"sort_order"`
...
func FromDomain(v *domain.ProductVariant) VariantResponse {
	...
	return VariantResponse{
		ID: v.ID, ProductID: v.ProductID, Name: v.Name, Slug: v.Slug,
		Attributes: v.Attributes, IsPublished: v.IsPublished, SortOrder: v.SortOrder,
		Images: images, CreatedAt: v.CreatedAt, UpdatedAt: v.UpdatedAt,
	}
}
