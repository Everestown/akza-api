...
type svc interface {
	...
	ListDeleted(ctx context.Context) ([]dto.CollectionResponse, error)
	...
	Restore(ctx context.Context, id int64) (*dto.CollectionResponse, error)
...
func (h *Handler) ListDeleted(c *gin.Context) {
	items, err := h.svc.ListDeleted(c.Request.Context())
	if err != nil { middleware.Err(c, err); return }
...
func (h *Handler) Create(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil { middleware.Err(c, validationErr(err)); return }
...
func (h *Handler) Restore(c *gin.Context) {
	id, ok := httputil.ParseID(c); if !ok { return }
	col, err := h.svc.Restore(c.Request.Context(), id)
	if err != nil { middleware.Err(c, err); return }
	middleware.OK(c, col)
}
...
func (h *Handler) ConfirmCover(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&body); err != nil { middleware.Err(c, validationErr(err)); return }
