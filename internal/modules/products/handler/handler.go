...
type svc interface {
	ListByCollection(ctx context.Context, collectionID int64, onlyPublished bool, p pagination.CursorPage) (pagination.PageResult[dto.ProductResponse], error)
	...
	GetByIDAdmin(ctx context.Context, id int64) (*dto.ProductResponse, error)
...
// ListByCollectionAdmin — admin: GET /admin/products?collection_id=123
func (h *Handler) ListByCollectionAdmin(c *gin.Context) {
	v := c.Query("collection_id")
	if v == "" { middleware.Err(c, apperror.Validation("collection_id query param required")); return }
	var cid int64
	if _, err := fmt.Sscanf(v, "%d", &cid); err != nil {
		middleware.Err(c, apperror.Validation("collection_id must be integer")); return
	}
	var p pagination.CursorPage; _ = c.ShouldBindQuery(&p)
	result, err := h.svc.ListByCollection(c.Request.Context(), cid, false, p)
	if err != nil { middleware.Err(c, err); return }
	middleware.Paginated(c, result.Data, result.Cursor, result.HasMore, result.Limit)
}
...
func (h *Handler) GetByID(c *gin.Context) {
	...
	p, err := h.svc.GetByIDAdmin(c.Request.Context(), id)
	if err != nil { middleware.Err(c, err); return }
	middleware.OK(c, p)
}
