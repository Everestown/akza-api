...
type repo interface {
	ListByCollection(ctx context.Context, collectionID int64, onlyPublished bool, p pagination.CursorPage) ([]domain.Product, error)
...
func build(items []domain.Product, limit int) pagination.PageResult[dto.ProductResponse] {
	responses := make([]dto.ProductResponse, len(items))
	for i, p := range items { responses[i] = dto.FromDomain(&p) }
	return pagination.BuildResult(responses, limit, func(r dto.ProductResponse) string {
		return pagination.EncodeCursor(r.ID, r.CreatedAt)
	})
}

func (s *Service) ListByCollection(ctx context.Context, collectionID int64, onlyPublished bool, p pagination.CursorPage) (pagination.PageResult[dto.ProductResponse], error) {
...
func (s *Service) GetByIDAdmin(ctx context.Context, id int64) (*dto.ProductResponse, error) {
...
func (s *Service) Update(ctx context.Context, id int64, req dto.UpdateProductRequest) (*dto.ProductResponse, error) {
	p, err := s.repo.FindByID(ctx, id)
	if err != nil { return nil, err }
	p.Title = req.Title; p.Description = req.Description
	p.Characteristics = req.Characteristics; p.Price = req.Price
	p.PriceHidden = req.PriceHidden; p.SortOrder = req.SortOrder
	if err = s.repo.Update(ctx, p); err != nil { return nil, err }
	resp := dto.FromDomain(p); return &resp, nil
}

func (s *Service) SetPublished(ctx context.Context, id int64, published bool) error {
...
func (s *Service) PresignCover(ctx context.Context, id int64, filename, ct string) (*dto.PresignResponse, error) {
	...
	key := storage.BuildKey("products", fmt.Sprintf("%d", id), filename)
